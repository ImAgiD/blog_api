...
#         user = User.objects.create_user(email='admin1@admin.com', password='1')
#         posts = [
#             Post(author=user, title='new_post'),
...
#         assert response.data[0]['title'] == 'new_post'
...
#     #     id = Post.objects.all()[0].id
#     #     url = reverse('post_detail', kwargs={'pk': id})
#     #     print(url)
...
#         assert Post.objects.filter(auhtor=user, title=data['title']).exists()
...
#         assert response.status_code == 200    
#         # в нашем случае этот тест/код не работает, т.к у нас есть логика на ограничение пользователей
